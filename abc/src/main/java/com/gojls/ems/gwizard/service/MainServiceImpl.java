...
public class MainServiceImpl implements MainService {
	...
	public Map<String, Object> getMain(Map<String, Object> map){
		...
		eventInfoModel.setGtEventType("EVT002");
		...
		List<EventUserModel> eventTopUserList = eventUserDaoOra.selectEventTopUserList(eventUserModel);
		...
		if(userSeq != null && !userSeq.equals("")) {
			...
			if(isUserCheck == 1) {
				...
				if(eventUserInfo == null) {	//이벤트가 없는 경우 도전자로 insert
					int startLevelSeq = 1;
					String nickNm = "";
					if(eventInfoModel.getEpisodeSeq() == 2) {
						startLevelSeq = 6;
						
						EventUserModel prevUserModel = new EventUserModel();
						prevUserModel.setEventSeq(2);
						prevUserModel.setUserSeq(userSeq);
						EventUserModel prevUserInfo = eventUserDaoOra.selectEventUser(prevUserModel);
						if(prevUserInfo != null) {
							nickNm = (prevUserInfo.getNickNm() == null)?"":prevUserInfo.getNickNm();
						}
					}
		...
		//동영상 리스트
		int groupCnt = 5;
		if(eventInfoModel.getEpisodeSeq() == 2) groupCnt = 7;
		...
		resultMap.put("eventTopUserList", eventTopUserList);
	...
	public Map<String, Object> selectEventUserNickNmCheck(EventUserModel eventUserModel){
		...
		eventUserModel.setEventUserSeq(eventUserInfo.getEventUserSeq());
		int nickNmCnt = eventUserDaoOra.selectEventUserNickNmCnt(eventUserModel);
		
		if(nickNmCnt == 0) {//동일한 별명이 없는 경우
			List<EventSwearFilterModel> eventSwearFilterList = eventUserDaoOra.selectEventSwearFilterCheck(eventUserModel);
			if(eventSwearFilterList == null || eventSwearFilterList.size() == 0) {	//욕설이 포함된 별명이 아닌 경우
				resultMap.put("RESULT", "SUCCESS");
			}else {	//욕설이 포함 된 경우
				resultMap.put("RESULT", "FAIL2");
				resultMap.put("swearWord", eventSwearFilterList.get(0).getWord());
			}
		}else {//동일한 별명이 있는 경우
	...
	public void updateEventUserNickNm(EventUserModel eventUserModel){
		...
		eventUserDaoOra.updateEventUserNickNm(eventUserModel);
